...
app.MapPost("/api/pipeline/run", async ([FromQuery] string accountNo, [FromQuery] string? targetDate) =>
{
    ...
    try
    {
        ...
        // ใช้เลขบัญชีลูกค้าที่ส่งมาทาง Query Parameter (กรอกทีละเลขบัญชี)
        List<string> rawAccounts = new List<string> { accountNo.Trim() };

        int headerCount = 0;
        int detailCount = 0;
        var summaryTrail = new List<object>();

        // append: true เพื่อเขียนต่อท้ายไฟล์เดิม รองรับการรันหลายรอบต่อเนื่องกัน
        using (var headerWriter = new StreamWriter(headerFileName, true, Encoding.UTF8))
        using (var detailWriter = new StreamWriter(detailFileName, true, Encoding.UTF8))
        {
            foreach (var rawAccount in rawAccounts)
            {
                ...
                // หากไม่มี frontaccount ใน DB ให้ฟอร์แมต 99 + account (แบบไม่มี dash) ด้วย
                if (string.IsNullOrEmpty(customerData.FrontAccount))
                {
...
public static class SbaDatabaseService
{
    ...
    private static async Task<bool> TestTableAsync(OdbcConnection conn, string tableName)
    {
        try
        {
            // ใช้ "SELECT FIRST 1" เป็น syntax ของ Informix ในการทดสอบตาราง
