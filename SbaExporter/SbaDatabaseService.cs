...
namespace SbaExporter
{
    ...
    public static class SbaDatabaseService
    {
        ...
        public static void DisableOdbcPooling()
        {
            try
            {
                // -6 = SQL_IS_INTEGER
                int result = SQLSetEnvAttr(IntPtr.Zero, SQL_ATTR_CONNECTION_POOLING, IntPtr.Zero, -6);
                Log.Information($"[ODBC] ปิดระบบ Connection Pooling สำเร็จด้วยรหัส: {result} (0 = SQL_SUCCESS)");
            }
            catch (Exception ex)
            {
                Log.Warning($"[ODBC] ไม่สามารถปิด Connection Pooling ได้: {ex.Message}");
            }
        }
        ...
        public static List<CustomerPipelineData> LoadAllPipelineDataFromSba(List<string>? specificAccounts = null)
        {
            ...
            try
            {
                using (var conn = new OdbcConnection(ConnectionString))
                {
                    ...
                    // 2. ดึงตารางที่เกี่ยวข้องมาทำ Cache บนแรม
                    ...
                    Log.Information($"[DB] ดึงข้อมูลดิบจาก SBA ขึ้นมาเก็บบน Memory สำเร็จ: {pipelineList.Count} รายการ");
                }
            ...
            {
                Log.Error($"[DB Error] ไม่สามารถโหลดข้อมูลได้: {ex.Message}");
                throw;
            }
            return pipelineList;
        }
